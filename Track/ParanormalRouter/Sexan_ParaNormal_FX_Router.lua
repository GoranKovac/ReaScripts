    ...
    for i = 1, #tbl do
        ...
        if type(tbl[i]) ~= "table" then
            ...
            if r.ImGui_BeginDragDropSource(ctx) then
                r.ImGui_SetDragDropPayload(ctx, 'DRAG ADD FX', table.concat({ path, os_separator, tbl[i] }))
                r.ImGui_Button(ctx, tbl[i])
...
local function CalcFxID(tbl, i)
    if tbl.DIFF then
        return 0x2000000 + tbl.C_ID + (tbl.DIFF * tbl.IDX)
    else
        return tbl.IDX - 1
    end
end

local function AddFxID(tbl, i)
    if tbl.DIFF then
...
local function CalcContainerWH(fx_items)
    ...
    local vol_button_offset = (def_btn_h + s_window_y / 2)
    ...
    for i = 1, #rows do
        ...
        if #rows[i] > 1 then
            for j = 1, #rows[i] do
                ...
                local h = fx_items[rows[i][j]].H and fx_items[rows[i][j]].H + s_spacing_y + vol_button_offset or
                    btn_total_size + vol_button_offset
                col_w = col_w + w
...
local function IterateContainer(depth, track, container_id, parent_fx_count, previous_diff, parent_guid)
    local sub_fx = {}
    ...
    for i = 1, c_fx_count do
        ...
        sub_fx[#sub_fx + 1] = {
            ...
            wetparam = wetparam,
            wet_val = wet_val,
            color = AUTO_COLORING and color or COLOR["n"]
        }

        if fx_type == "Container" then
            local s_sub_fx, s_total_w, s_total_h = IterateContainer(depth + 1, track, fx_id, c_fx_count, diff, fx_guid)
            ...
            s_sub_fx[0] = {
                ...
                wetparam = wetparam,
                wet_val = wet_val,
                color = AUTO_COLORING and color or COLOR["cont_header"]
            }
...
local function CheckFX()
    ...
    for i = 1, TRACK_FX_CNT do
        ...
        if fx_type == "Container" then
            sub_fx, cont_w, cont_h = IterateContainer(0, TRACK, i, TRACK_FX_CNT, 0, fx_guid)
            sub_fx[0] = {
                DIFF = (TRACK_FX_CNT + 1),
                C_ID = i,
                C_IDX = i,
...
local function DrawButton(btn_sx, btn_sy, item_w, item_h, name, color)
    ...
    --r.ImGui_DrawList_AddTextEx(draw_list, nil, FONT_SIZE, (btn_sx + (item_w / 2) - label_size / 2) + 1,
    --    (btn_sy + ((item_h / 2)) - FONT_SIZE / 2) + 1, 0x000000ff, name)
    r.ImGui_DrawList_AddTextEx(draw_list, nil, FONT_SIZE, btn_sx + (item_w / 2) - label_size / 2,
        btn_sy + ((item_h / 2)) - FONT_SIZE / 2, font_color, name)
end
...
local function AddFX_S(tbl)
    ...
    if r.ImGui_InvisibleButton(ctx, "+", add_bnt_size, def_btn_h) then
        --local fx_id = CalcFxID(tbl, #tbl + 1, -1000)
        --local fx_id = CalcFxID(tbl, #tbl + 1)
        local fx_id = AddFxID(tbl[0], #tbl + 1)
        FX_ID = { fx_id }
        OPEN_POPUP = true
    end
    r.ImGui_PopID(ctx)
...
local function ButtonAction(tbl, i)
    ...
    if ALT then
        ...
        r.TrackFX_SetOpen(TRACK, fx_id, not r.TrackFX_GetOpen(TRACK, fx_id))
    end
end
...
local function SwapTargetDragDrop(tbl, i)
    if r.ImGui_BeginDragDropTarget(ctx) then
        ...
        r.ImGui_EndDragDropTarget(ctx)
        if ret then
            local SRC_CONT, DST_CONT
            local src_tbl_0_guid, src_id = payload:match("(.+),(.+)")
            local src_tbl_0, src_i = InTbl(PLUGIN, src_tbl_0_guid), tonumber(src_id)
            local dst_tbl_0, dst_i = tbl[0], i

            -- SRC CONTAINER
            if src_i == 0 then
                SRC_CONT = src_tbl_0
                src_i = src_tbl_0.C_IDX
                src_tbl_0 = InTbl(PLUGIN, src_tbl_0.P_GUID)
            end
            -- DST CONTAINER
            if dst_i == 0 then
                DST_CONT = dst_tbl_0
                dst_i = dst_tbl_0.C_IDX
                dst_tbl_0 = InTbl(PLUGIN, dst_tbl_0.P_GUID)
            end

            -- DO NOT MOVE CONTAINER INTO ITS CHILDS
            if SRC_CONT then
                if SRC_CONT.C_ID == dst_tbl_0.C_ID then return end
            end
            -- DO NOT MOVE CONTAINER INTO ITS CHILDS
            if DST_CONT then
                if DST_CONT.C_ID == src_tbl_0.C_ID then return end
            end

            SwapFX(src_tbl_0, dst_tbl_0, src_i, dst_i)
        end
    end
end
...
local abs = math.abs
local HOVER
local function ParaVolume()
    local x, y = r.ImGui_GetMouseDelta(ctx)
    y = y ~= 0 and y // abs(y) / 100 or 0
    local wet = r.TrackFX_GetParam(TRACK, HOVER[2], HOVER[1].wetparam)
    r.ImGui_VSliderInt(ctx, "##V", 35, 100, (wet * 100) // 1, 0, 100)
    r.TrackFX_SetParam(TRACK, HOVER[2], HOVER[1].wetparam, wet - y)
    if not r.ImGui_IsMouseDown(ctx, 0) then
        HOVER, PX, PY = nil, nil, nil
        r.ImGui_CloseCurrentPopup(ctx)
    end
end
...
local function AddFXControls(tbl, i, item_w)
    ...
    r.ImGui_SetCursorPos(ctx, x + item_w - 11, y - s_spacing_y + def_btn_h / 2) -- - def_btn_h)
    local btn_sx, btn_sy = r.ImGui_GetCursorScreenPos(ctx)
    r.ImGui_PushID(ctx, "VOL" .. tbl[i].guid)
    r.ImGui_InvisibleButton(ctx, "##", 22, def_btn_h / 1.5)

    local val = string.format("%.0f", (tbl[i].wet_val * 100) / 1)
    DrawButton(btn_sx, btn_sy, 22, def_btn_h / 1.5, val, COLOR["n"], (ALT and r.ImGui_IsItemHovered(ctx)), 10)
    r.ImGui_PopID(ctx)

    if not HOVER then
        PX, PY = r.ImGui_GetMousePos(ctx)
        HOVER = r.ImGui_IsItemActive(ctx) and { tbl[i], CalcFxID(tbl[0], i) } or nil
    end

    if r.ImGui_IsItemActive(ctx) then OPEN_VOLUME = true end
end
...
local function Draw(tbl, x)
    for i = 0, #tbl do
        ...
        if tbl[i].type ~= "Container" then
            ...
            local color = (ALT and r.ImGui_IsItemHovered(ctx)) and COLOR["del"] or tbl[i].color
            -- (i == 0 and COLOR["cont_header"] or COLOR["n"])s
            --r.ShowConsoleMsg(tbl[i].color .. "\n")
            DrawButton(btn_sx, btn_sy, item_w, item_h, tbl[i].name, tbl[i].color, (ALT and r.ImGui_IsItemHovered(ctx)))
...
local function Popups()
    ...
    if OPEN_VOLUME then
        OPEN_VOLUME = nil
        if not r.ImGui_IsPopupOpen(ctx, "OPEN_VOLUME") then
            r.ImGui_OpenPopup(ctx, "OPEN_VOLUME")
        end
    end

    if HOVER then
        r.ImGui_SetNextWindowPos(ctx, PX - 18, PY - s_window_y - (100 - (HOVER[1].wet_val * 100) // 1))
    end
    if r.ImGui_BeginPopup(ctx, "OPEN_VOLUME") then
        ParaVolume()
        r.ImGui_EndPopup(ctx)
    end
end
