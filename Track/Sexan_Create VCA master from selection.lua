...
-- @description Create VCA Master from selection.lua
-- @author SeXan
...
local function scan_groups()
    ...
    for i = 0, cnt_tr - 1 do
        ...
        for k = 1, #vca_group do
            if reaper.GetSetTrackGroupMembership(tr, "VOLUME_VCA_MASTER", 0, 0) == 2 ^ (k - 1) or reaper.GetSetTrackGroupMembershipHigh(tr, "VOLUME_VCA_MASTER", 0, 0) == 2 ^ ((k - 32) - 1) then
                cnt =
                    cnt + 1
            end
...
function create_VCAs()
    ...
    for i = group_range, #vca_group do
        ...
        if vca_group[i] == 0 then
            if i > 32 then
                high = true
                group = reaper.GetSetTrackGroupMembershipHigh
                free_group = 2 ^ ((i - 32) - 1)
            else
                group = reaper.GetSetTrackGroupMembership
                free_group = 2 ^ (i - 1)
            end
    ...
    if popup == 0 then
        ...
        reaper.GetSetProjectInfo_String( 0, "TRACK_GROUP_NAME:" .. group_idx,"VCA: " .. cnt, true )
    else
        ...
        if ret then
            ...
            reaper.GetSetProjectInfo_String( 0, "TRACK_GROUP_NAME:" .. group_idx,"VCA: " .. name, true )
