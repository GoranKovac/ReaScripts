...
function Element:zoneIN(x, y)
  ...
  if y >= self.y and y <= self.y + range2 then return {"T", self.y, self.h} end
  if y <= self.y + self.h and y >= (self.y + self.h) - range2 then return {"B", self.y, self.h} end
  
  if x > (self.x + range2) and x < (self.x + self.w - range2) then
    if y > self.y + range2 and y < (self.y + self.h) - range2 then return {"C",self.x, self.w, self.y} end
  end
end
...
function Element:mouseClick()
  return mouse.l_click and
  self:pointIN(mouse.ox, mouse.oy)         
end
------------------------
...
local m_bm = reaper.JS_LICE_CreateBitmap(true, 1, 1); reaper.JS_LICE_Clear(m_bm, 0x33005511)
local last_num = 0
function Menu:draw_body(tbl)
  ...
  self.x, self.y, self.w, self.h = self.x + round(self.w /2), self.y , round(self.w /2), 15--math.ceil(h/4)
