...
function zone(z)
  local rx, ry   = m_x - last_x, m_y - last_y
...
function item_blit(item, as_start, as_end , pos)
  ...
  if tsStart < item_start and tsEnd > item_start and tsEnd < item_dur then
    ...
    new_start, new_item_lenght, offset = (pos ~= nil) and ((item_start-tsStart) + pos) or item_start, tsEnd - item_start, 0
  ...
  elseif tsStart < item_dur and tsStart > item_start and tsEnd > item_dur then
    ...
    new_start, new_item_lenght, offset = (pos ~= nil) and pos or tsStart , item_dur - tsStart, (tsStart - item_start)
    return new_start, new_item_lenght, offset, item
  elseif tsStart >= item_start and tsEnd <= item_dur then
    ------ IF BOTH TS START AND TS END ARE IN ITEM
    new_start, new_item_lenght, offset = (pos ~= nil) and pos or tsStart , tsEnd - tsStart, (tsStart - item_start)
    return new_start, new_item_lenght, offset, item
  elseif tsStart <= item_start and tsEnd >= item_dur then -- >= NEW
    ------ IF BOTH TS START AND END ARE OUTSIDE OF THE ITEM
    new_start, new_item_lenght, offset = (pos ~= nil) and ((item_start-tsStart) + pos) or item_start, item_lenght, 0
    return new_start, new_item_lenght, offset, item
  end
...
function paste(tbl, mouse_tr, mouse_time_pos, active_as)
  if not mouse_tr then return end -- DO NOT PASTE IF MOUSE IS OUT OF ARRANGE WINDOW
  
  reaper.PreventUIRefresh(1)
  local first_as_tr_in_all = find_highest_tr()
   
  if reaper.ValidatePtr(mouse_tr, "TrackEnvelope*") then mouse_tr = reaper.Envelope_GetParentTrack( mouse_tr ) end -- IF MOUSE TRACK IS ENVELOPE GETS ITS PARENT
  ...
  for i = 1, #tbl do 
    ...
    for j = 1 ,#tbl2.info do
       if tbl2.info[j].items then
        local as_tr = tbl2.info[j].track
        local offset , tr = GetTrackOffset(as_tr, mouse_tr, first_as_tr_in_all, tbl2.info[#tbl2.info].track)
        
        if offset and offset > 0 then 
          for t = 1, offset do reaper.InsertTrackAtIndex(( reaper.GetNumTracks() ), true ) end-- IF THE TRACKS ARE BELOW LAST TRACK OF THE PROJECT CREATE HAT TRACKS
        end
        
        for k = 1, #tbl2.info[j].items do
          local item = tbl2.info[j].items[k]
          create_item(item, tr, tbl2.time_start, tbl2.time_end, pos) -- CREATE ITEMS AT NEW POSITION
        end
