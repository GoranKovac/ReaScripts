...
function Round(n)
	return n % 1 >= 0.5 and ceil(n) or floor(n)ŽŽ
end
...
local function GDIBlit(destination, x, y, w, h)
	local zoom_lvl,
		Arr_start_time,
		Arr_end_time,
		Arr_pixel,
	...
	for i = 1, #data do
		if data[i].items then
			...
			for j = 1, #data[i].items do
				...
				GDIBlit(dc, item_start, (item_t + item_bar), item_lenght, item_h - 19)

				ghosts[item_ghost_id] = {
					--	TODO: BETTER NAMING l-len, s-start
					bm = bm,
					dc = dc,
					h = item_h,
		...
		elseif data[i].env_points then
			...
			GDIBlit(dc, as_start, env_t, (as_end - as_start), env_h)
...
function GetSelectionInfo(tbl)
	...
	local sel_info = GetTrackData(tracks, area_start, area_end) -- GATHER ALL INFO
	return sel_info
end
...
function DrawItemGhosts(item_data, item_track, as_start, as_end, pos_offset, first_track)
	...
	if TBH[offset_track] then -- THIS IS NEEDED FOR PASTE FUNCTION OR IT WILL CRASH
		...
		for i = 1, #item_data do
			...
			local item_start, item_lenght = item_blit(item, as_start, as_end, pos_offset)
			local item_ghost_id = tostring(item) .. as_start
...
function find_highest_tr(job)
	...
	for i = 1, #as_tbl do
		...
		for j = 1, #tbl.sel_info do
			-- if not tbl.sel_info[j].items and not tbl.sel_info.env_name and job == "track" then
			--local as_tr = tbl.sel_info[j].track
			-- return as_tr
