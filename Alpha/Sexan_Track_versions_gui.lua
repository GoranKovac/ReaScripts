...
function update_menubox(val)
  for i = 1, #GUI.elms.Menubox1.optarray do
    if val == GUI.elms.Menubox1.optarray[i] then GUI.Val("Menubox1",i) end
  end
end
...
function populate_listbox(tbl)
  ...
  if tab == 2 then
    ...
    if env_list then GUI.elms.Menubox1.optarray = env_list else GUI.elms.Menubox1.optarray = {"Empty"} end
  ...
  if tbl then
    local num,data
    if tab == 1 then
      if not tbl.data then GUI.elms.Listbox1:redraw() return end
      num,data = tbl.data.num,tbl.data
    elseif tab == 2 then
      local last = GUI.elms.Menubox1.optarray[GUI.Val("Menubox1")] -- SELECTED MENUBOX
      if not tbl[last] then GUI.elms.Listbox1:redraw() return end -- NO VERSIONS IN SELECTED MENUBOX
      num,data = tbl[last].num,tbl[last] -- GET ONLY ENVELOPES THAT HAVE SAME NAME AS MENUBOX
    elseif tab == 3 then
      if not tbl.fx then GUI.elms.Listbox1:redraw() return end
...
function delete()
  ...
  if tab == 1 then
    ...
    if not tbl.data then populate_listbox(tbl) return end
    if SHOW == 1 then sort_fipm(tbl.guid) end -- MAYBE A BETTER WAY JUST TO DELETE ITEMS WHICH HAS NO NUM CORDINATE
  ...
  elseif tab == 2 then
    ...
    if not tbl[cur_env] then populate_listbox(tbl) return end
    restore_envelope(tbl.guid,cur_env,tbl[cur_env].num)
  elseif tab == 3 then
    delete_from_table(tbl.fx,cur_val,cur_tr,tab)
    if not tbl.fx then populate_listbox(tbl) return end
...
function rename()
  ...
  elseif tab == 2 then
    if not tbl[cur_env] then return end
...
local function comp(param)
  ...
  if param then cur_comp_id = reaper.genGuid() else cur_comp_id = nil end
...
function merge()
  local tbl = find_guid(cur_tr)
  if tbl.stored_num ~= tbl.num then tbl.data[tbl.stored_num].chunk = getTrackItems(reaper.BR_GetMediaTrackByGUID( 0, cur_tr)) end -- PREVENT STORING CURRENT VERSION (CHUNK IS THE SAME)
end
...
local function view_ts(param)  
  ...
  for i = 1, #tracks do
    ...
    if param then
      if GUI.Val("Tabs1") == 1 then
        if not track.data then return end
        track.stored_num,track.stored_version = track.data.num, track.data[track.data.num].ver_id 
      elseif GUI.Val("Tabs1") == 2 then 
        if not track[cur_menu] then return end       
        track.stored_num,track.stored_version = track[cur_menu].num, track[cur_menu][track[cur_menu].num].ver_id
      elseif GUI.Val("Tabs1") == 3 then return
      end
    else
      if GUI.Val("Tabs1") == 1 then
        if track.stored_num ~= track.data.num then restoreTrackItems(track.guid, track.stored_num) end -- DO NOT RESTORE SAME VERSIONS
      ...
      elseif GUI.Val("Tabs1") == 2 then
        if track.stored_num ~= track[cur_menu].num then restore_envelope(track.guid,cur_menu,track.stored_num) end  -- ADD WAY NOT TO TRIGGER SAME VERSION AGAIN
  ...
  reaper.PreventUIRefresh(-1)
  if param then GUI.elms.Merge.z = 11 else GUI.elms.Merge.z = 10 end  
...
function show_all(param)
  ...
  for i = 1, #tracks do
    ...
    if param then sort_fipm(tracks[i]) else clear_muted_items(tracks[i]) end
  end  
...
GUI.New("Listbox1", "Listbox", {z = 9,x = 108,y = 108,w = 195,h = 200,list = "",multi = false,caption = "",font_a = 3,font_b = 4,color = "txt",col_fill = "elm_fill",bg = "elm_bg",cap_bg = "wnd_bg",shadow = true,pad = 4,last_num = "",last_env_num = ""})
function GUI.elms.Listbox1:onmouseup()
  GUI.Listbox.onmouseup(self)
  local tab = GUI.Val("Tabs1")
  local cur_val = GUI.Val("Listbox1") -- GET CURRENT VALUE
  local tbl = find_guid(cur_tr)
  if #self.list == 0 then return end   -- IF EMPTY DO NOTHING  
  reaper.PreventUIRefresh(1)
...
GUI.New("Tabs1", "Tabs", {z = 11,x =108,y = 67, w = 912.0,caption = "Tabs1",optarray = {"Lane", "Envelope", "FX"},tab_w = 48,tab_h = 20,pad = 8,font_a = 3,font_b = 4,col_txt = "txt",col_tab_a = "wnd_bg",col_tab_b = "tab_bg",bg = "elm_bg",fullwidth = false})
...
GUI.New("Menubox2", "Menubox", {z = 5,x = 168,y = 88,w = 80,h = 20,caption = "Copy To -",optarray = {"Empty"},retval = 1,font_a = 3,font_b = 4,col_txt = "txt",col_cap = "txt",bg = "wnd_bg",pad = 4,noarrow = false,align = 0})

function GUI.elms.Menubox2:onmouseup()
  GUI.Menubox.onmouseup(self)
  if self.optarray[1] == "Empty" then return end
...
function groups(params)
  ...
  if TrackTB.groups then GUI.elms.Listbox2.list = TrackTB.groups[group] end
...
function add_to_group()
  ...
  local group = GUI.Val("GroupList")
  if not TrackTB.groups then TrackTB.groups = {} end
  if TrackTB.groups[group] == nil then TrackTB.groups[group] = {} end -- create table
  for i = 1, reaper.CountSelectedTracks() do 
    local tr = reaper.GetTrackGUID(reaper.GetSelectedTrack(0,i-1))    
    if not has_undo(TrackTB.groups[group], tr) then TrackTB.groups[group][#TrackTB.groups[group]+1] = tr end -- DO NOT ADD SAME TRACKS TO TABLE
  end
  save_tracks()
...
function remove_from_group()
  ...
  if #TrackTB.groups[group] == 0 then TrackTB.groups[group] = nil end -- IF TABLE IS EMPTY DELETE TABLE
  if #TrackTB.groups == 0 then TrackTB.groups = nil end
end

GUI.New("Listbox2", "Listbox", {z = 6,x = 100,y = 80,w = 129,h = 205,list = {},multi = false,caption = "",font_a = 3,font_b = 4,color = "txt",col_fill = "elm_fill",bg = "elm_bg",cap_bg = "wnd_bg",shadow = true,pad = 4,last_num = "",last_env_num = ""})
GUI.New("GroupList", "Radio", {z = 6,x = 35,y = 45,w = 50,h = 240,caption = "GROUP",optarray = {"1","2","3","4","5","6","7","8","9"},dir = "v",font_a = 2,font_b = 3,col_txt = "txt",col_fill = "elm_fill",bg = "wnd_bg",frame = true,shadow = true,swap = nil,opt_size = 20})
GUI.New("Add",    "Button", {z = 6,x = 100,y = 48,w = 58,h = 24,caption = "Add",      font = 3,col_txt = "txt",func = add_to_group})
GUI.New("Remove", "Button", {z = 6,x = 170,y = 48,w = 58,h = 24,caption = "Remove",   font = 3,col_txt = "txt",func = remove_from_group})
GUI.New("Window", "Window",   {z = 7, x = 15,y = 8, w = 235, h = 300,caption = "Multi Edit Groups",z_set = {6,7},center = true})
------------------------------------------------------------------------------------------------------------------------------
GUI.New("Label1", "Label",      {z = 9, x = 20,y = 11,caption = "",font = 2,color = "txt",bg = "wnd_bg",shadow = true})
GUI.New("Frame1", "Frame",      {z = 11,x = 15,y = 8,  w =233,h = 26,shadow = false,fill = false,color = "elm_frame",bg = "wnd_bg",round = 0,text = "",txt_indent = 0,txt_pad = 0,pad = 4,font = 4,col_txt = "txt"})
GUI.New("Groups", "Button",     {z = 11,x = 14,y = 42, w = 78,h = 24,caption = "Groups", font = 3,col_txt = "txt",func = groups,params = {true}})
GUI.New("NewVersion", "Button", {z = 11,x = 14,y = 88, w = 78,h = 24,caption = "New Version", font = 3,col_txt = "txt",func = new_version, params = {GUI.Val("Tabs1")}})
GUI.New("Duplicate",  "Button", {z = 11,x = 14,y = 118,w = 78,h = 24,caption = "Duplicate",   font = 3,col_txt = "txt",func = duplicate})
GUI.New("Delete",     "Button", {z = 11,x = 14,y = 158,w = 78,h = 24,caption = "Delete",      font = 3,col_txt = "txt",func = delete})
GUI.New("Rename",     "Button", {z = 11,x = 14,y = 188,w = 78,h = 24,caption = "Rename",      font = 3,col_txt = "txt",func = rename})
GUI.New("Copy_To",    "Button", {z = 11,x = 14,y = 218,w = 78,h = 24,caption = "Copy To",     font = 3,col_txt = "txt",func = to,params = {true}})
GUI.New("View_TS",    "Button", {z = 11,x = 14,y = 258,w = 78,h = 24,caption = "View TS",     font = 3,col_txt = "txt",func = view_ts, params = {true}})
GUI.New("Show_All",   "Button", {z = 11,x =108,y = 42, w = 64,h = 20,caption = "Show All",    font = 3,col_txt = "txt",func = show_all,params = {true}})
GUI.New("Comp",       "Button", {z = 11,x =184,y = 42, w = 64,h = 20,caption = "Comp",        font = 3,col_txt = "txt",func = comp,params = {true}})
GUI.New("Merge",      "Button", {z = 10,x = 14,y = 288,w = 78,h = 24,caption = "Merge",       font = 3,col_txt = "txt",func = merge})

GUI.New("TEST",       "Button", {z = 10,x =224,y = 42, w = 64,h = 20,caption = "test",        font = 3,col_txt = "txt",func = test})


function GUI.elms.GroupList:onmouseup()
  GUI.Radio.onmouseup(self)
  local num = GUI.Val("GroupList")
  if TrackTB.groups[num] == nil then GUI.elms.Listbox2.list = {}
  else
...
function mouse_click(click,tbl,mouse_ver)
  if click == 1 then down = true   
    if not hold then
      ...
      if mouse_ver then tbl.data.num = mouse_ver end
      hold = true
    end 
  end
  if down then
...
function menu(tbl,mouse_ver,job,fipm)
  ...
  local tmp1,tmp2
  if GUI.elms.Comp.state == 1 then tmp1 = "!" else tmp1 = "" end
  if GUI.elms.Copy_To.state == 1 then tmp2 = "!" else tmp2 = "" end
  if tbl then d_name = GUI.elms.Listbox1.list[tbl.data.dest] end
  ...
  if not job then
    ...
    for i = 1 ,#GUI.elms.Listbox1.list do
      ...
      if i == cur_val then tmp_str = "!" .. tmp_str end
      str = str .. "|" .. tmp_str
    end
    if tbl then cur_name = GUI.elms.Listbox1.list[cur_val] ..  str .. "|<|" end
    menu_options[1] = ">" ..  menu_options[1] .. " - " .. cur_name
  end
  
  if tbl then num = tbl.data.num else num = 0 end
  if not job then num = mouse_ver end  
   
  local m_num = gfx.showmenu(table.concat(menu_options,"|"))  
  if not tbl then m_num = m_num - 1 end
  if m_num == 0 then return end -- IF NOTHING IS CLICKED THEN RETURN
  if job then m_num = #GUI.elms.Listbox1.list - m_num end
  m_num = (m_num * -1) + 1
  ...
  if tbl then populate_listbox(tbl) end
end
...
local function Main()
  ...
  if last_tr ~= cur_tr then
    ...
    if cur_tbl then GUI.Val("Label1",update_txt_elements(cur_tr)) else GUI.Val("Label1","") end -- UPDATE LABEL ELEMENT TO TRACK NAME
    last_tr = cur_tr
  end
  ...
  if cur_tbl and cur_tbl.data and fipm == 1 then
    ...
    if tr_y_start and x > x_view_start and (y >= tr_y_start and y <= tr_y_end) and window == "arrange" then Acur_Y = y - tr_y_start else Acur_Y = nil end  
  ...
  if proj_change_count > last_proj_change_count then
    ...
    if last_redo == nil then last_redo = "AAAAAAA" end
    if last_redo:find("RTV") then unpack_undo(last_redo) end -- UNDO SYSTEM   
    ...
    elseif last_action:find("time selection change") and not last_redo:find("rtv") and COMP == 1 and fipm == 1 then comping({cur_tr},ver_under_mouse)
    elseif last_action:find("time selection change") and TO == 1 and fipm == 1 then get_items_for_destination(cur_tr,ver_under_mouse)
    -------------------------------------------------------------------------------------------------------------
    elseif last_action:find("change track free item positioning mode") then show_all(to_bool(fipm)) GUI.elms.Show_All:redraw() -- UPDATE FIMP IF ENABLED VIA TRACK RIGHT CLICK
    elseif last_redo:find("change track item positioning mode") then show_all(to_bool(fipm)) GUI.elms.Show_All:redraw()
    -------------------------------------------------------------------------------------------------------------
    end
    --------------------------------------------------
...
if not is_open then GUI.Init() end
GUI.func = Main
GUI.freq = 0
--GUI.freq = 0.05
read_from_file()
if not is_open then GUI.Main() end
