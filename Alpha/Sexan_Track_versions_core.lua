...
function unpack_undo(string)
  ...
  for i in string.gmatch(string, "[^%:]+") do 
    if i ~= "RTV" then undo[#undo+1] = i end 
  end
  local _, chunk = has_id(find_guid(undo[1]), undo[2]) -- GET CHUNK BY VER ID
  for i in string.gmatch(undo[3], "[^%,]+") do nums[#nums+1]= i end
...
function create_fx(guid,tab,job,duplicate)
  ...
  elseif job == "D" then name = naming(find_guid(guid),job,duplicate,"fx") -- CREATE DUPLICATE NAMES
  end  
  local data = {chunk = fx_chunk, name = name}-- .. #tbl.fx+1}  
  if not find_guid(guid) then
    TrackTB[#TrackTB+1] = {guid = guid, fx = {fx_num = 1, data}}
    goto JUMP
  else
    local version = find_guid(guid)
    if not version.fx then version.fx = {} end
...
function get_fx_chunk(guid)
  ...
  if not track_chunk:find("<FXCHAIN") then return end -- DO NOT ALLOW CREATING FIRST EMPTY FX
  local fx_start = track_chunk:find("<FXCHAIN")
  local fx_end = track_chunk:find("<ITEM")
  if not fx_end then fx_end = - 6 else fx_end = fx_end - 4 end
...
function edit_group_track_envelope()
  ...
  if not sel_points then return end
      ...
      if tr_group[i] ~= track_guid then -- DO FOR OTHER TRACKS
        ...
        local br_env = reaper.BR_EnvAlloc( env, true ) -- get
          --if range_start then
            ...
            for j = 1, #all_pt do
              if point.time == all_pt[j].time then sel = true all_pt[j].selected = sel else sel = false end
...
function get_time_sel()
  ...
  if t_start == 0 and t_end == 0 then return false else return t_start, t_end end
end
-------------------------------------------------------------------
---  Function FIND VERSION BASED ON MOUSE POSITION IN THE TRACK  ---
--------------------------------------------------------------------
function get_mouse_ver(tbl,cY)
  if not tbl or not cY then return end
...
function get_tracks()
  ...
  for i = 1, sel_tr_count do
    ...
    local tr_guid = reaper.GetTrackGUID(tr)
      if get_folder(tr) then tracks = get_folder(tr) -- FOLDER
      else tracks[#tracks+1] = tr_guid  -- SINGLE TRACK OR SELECTED TRACKS
      end
  end
  return tracks
end
...
function update_txt_elements(guid)
  ...
  if folder == 1 then lbl_name = lbl_name .. " - " .. "FOLDER" end
  return lbl_name  
end
----------------------------------
...
local function count(tbl, val)
  ...
  for i = 1 , #tbl do
    if val == "V" then
      if string.len(tbl[i].name) == 3 and tbl[i].name:find(val)then cnt = cnt + 1 end -- VERSIONS V01
    elseif val == "Comp" then
      if tbl[i].name:match(val) then cnt = cnt + 1 end -- VERSIONS Comp01 
    else
      if tbl[i].name:gsub('%-', ''):match(val:gsub('%-', '')) then cnt = cnt + 1 end -- DUPLICATES V01-D01
    end    
  end
return cnt
end
...
function has_id(tab, val)
  for i = 1 , #tab.data do
    ...
    if in_table == val then return i, tab.data[i].chunk end
  end
return false
end
----------------------------
...
function has_env(tbl, val)
  for k, v in pairs(tbl) do
    local in_table = k
    if in_table == val then return true end
  end
return false
end
-------------------------------------------------------------
...
function check_for_emtpy_table(tbl)
  ...
  local cnt = 0
  for k,v in pairs(tbl)do
    if type(v) == "table" then cnt = cnt + #v end
  end
  return cnt
end
------------------------------------
...
function delete_from_table(tbl,val,guid,tab,cur_env)
  ...
  if tab == 1 then
    if cur_tbl.data.num >= #cur_tbl.data then cur_tbl.data.num = #cur_tbl.data else cur_tbl.data.num = cur_tbl.data.num end -- CHANGE TRACK/CHILD NUM
    if cur_tbl.data.dest > cur_tbl.data.num then cur_tbl.data.dest = cur_tbl.data.num end
    if #cur_tbl.data == 0 then cur_tbl.data = nil end -- DELETE ITEM TABLE IF EMPTY
  elseif tab == 2 then
    if cur_tbl[cur_env].num >= #cur_tbl[cur_env] then cur_tbl[cur_env].num = #cur_tbl[cur_env] else cur_tbl[cur_env].num = cur_tbl[cur_env].num end
    if #cur_tbl[cur_env] == 0 then cur_tbl[cur_env] = nil end -- DELETE ENV TABLE IF EMPTY
  elseif tab == 3 then
    if cur_tbl.fx.fx_num >= #cur_tbl.fx then cur_tbl.fx.fx_num = #cur_tbl.fx else cur_tbl.fx.fx_num = cur_tbl.fx.fx_num end -- CHANGE TRACK/CHILD NUM
    if #cur_tbl.fx == 0 then cur_tbl.fx = nil end -- DELETE FX TABLE IF EMPTY
  end
  
  if check_for_emtpy_table(cur_tbl) == 0 then table.remove(TrackTB,num) end -- DELETE CURRENT TABLE ONLY IF THERE IS NO DATA INSIDE (ONLY GUID LEFT)  
  save_tracks()
end
-----------------------------------------------------
...
function get_num_from_cordinate(data,item) -- gets num value from table if selected item guid is found
  ...
  for i = 1, #data do
    for j = 1, #data[i].chunk do
      ...
      if item_guid == stored_guid then return i end
    end
  end
end
-------------------------------------------------------
...
function get_num_from_selected_item(item,tbl)
  local num  
  if item and get_num_from_cordinate(tbl.data,item) ~= nil then num = get_num_from_cordinate(tbl.data,item) tbl.num = num  mute_view(tbl,num) end  
  return num
end
--------------------------------------------------------
---  Function MUTE VIEW (VIEW ALL VERSION AT ONCE   ---
...
function update_fipm(tbl,override)
  ...
  if override then last_H = override end -- WE NEED TO UPDATE FIPM IN SOME SCENARIOS SO WEE WILL OVERRIDE THE CHECKING (MAKE IT PASS THE FUNCTION)
...
function pattern(chunk,guid)
  ...
  for i = 1 , #patterns do chunk = string.gsub(chunk, patterns[i], "") end -- remove SEL part of the chunk  
  if guid then chunk = string.gsub(chunk, guid, "") end 
  return chunk
end
---------------------------------
...
function getTrackItems(track,job)
  ...
  for i = 1, num_items, 1 do items[#items+1] = reaper.GetTrackMediaItem(track, i-1) end
  for i = 1, #items do
...
local function select_items(track,SEL)
  ...
  for i = 1 , #items do reaper.SetMediaItemSelected(items[i], SEL) end
  reaper.UpdateArrange()
end
----------------------------
---  BUTTONS FUNCTIONS   ---
...
local function new_empty(guid)
  ...
  for i = 1 , #childs do
    ...
    if not child.data then return end
...
function naming(tbl,string,dupli,job,cur_env)  
  ...
  if not tbl then return "Main" end   -- FIRST VERSION ON SELECTED TAB
  if job == "track" then data = tbl.data elseif job == "env" then data = tbl[cur_env] elseif job == "fx" then data = tbl.fx end
  if not data then return "Main" end -- FIRST VERSION IN OTHER TABS
...
function set_env_points(env,tbl)
  ...
  for i = 1, #tbl do
    if VIEW == 0 then
      ...
      if tbl[i].time >= ts_start and tbl[i].time <= ts_end then
        if not set_start then reaper.InsertEnvelopePoint( env, ts_start+0.01, tbl[i].value, tbl[i].shape, tbl[i].tension, tbl[i].selected, true) set_start = true end
        if not set_end then
          if math.floor(tbl[i].time) == math.floor(ts_end) then -- IF VALUES ARE CLOSE TO EACH OTHER (SAME)
            reaper.InsertEnvelopePoint( env, ts_end-0.01, tbl[i].value, tbl[i].shape, tbl[i].tension, tbl[i].selected, true)
            set_end = true 
          end
        end        
        reaper.InsertEnvelopePoint( env, tbl[i].time, tbl[i].value, tbl[i].shape, tbl[i].tension, tbl[i].selected, true)
  ...
  if VIEW == 1 then select_deselect_env_points(env,false) end
...
function select_deselect_env_points(env,SEL,r_start,r_end)
  ...
  for i = 0, env_point_count - 1 do
    ...
    points[#points+1] = {retval = retval, time = time, value = value , shape = shape, tension = tension, selected = false }
    end_time = time + 0.0001-- GET LAST POINT TIME + add small number to last point (ITS NOT DELETED BY DEFAULT)
    if SEL then reaper.SetEnvelopePoint( env, i, timeIn, valueIn, shapeIn, tensionIn, SEL, noSortIn ) end -- SELECT OR DESELECT ENVELOPE (SEL = FALSE-TRUE)    
    all_points[#all_points+1] = {retval = retval, time = time, value = value , shape = shape, tension = tension, selected = false }
    if selected == true then
      sel_points[#sel_points+1] = {retval = retval, time = time, value = value , shape = shape, tension = tension, selected = false }
    end
    if r_start then
      if time >= r_start and time <= r_end then
        range_points[#range_points+1] = {retval = retval, time = time, value = value , shape = shape, tension = tension, selected = false }
      end
    end
  end
  if #all_points == 0 then all_points = nil end
  if #sel_points == 0 then sel_points = nil end
  if #range_points == 0 then range_points = nil end
...
function get_envelope_track(guid,cur_env)
  ...
  for i = 0, envs-1 do
    ...
    if env_name == cur_env then break end
  end
  if #env_tbl == 0 then env_tbl = nil end
...
function get_env(guid,cur_env)
  ...
  if sel_env then
    ...
    local env_track, index, index2 = reaper.Envelope_GetParentTrack( sel_env ) -- GET TRACK OF SELECTED ENVELOPE
    local env_track_guid = reaper.GetTrackGUID( env_track )
    if env_track_guid == guid and cur_env == sel_env_name then env = sel_env end -- GUID IS SAME,NAME MATCHES MENUBOX
  end  
  if not env then return end    
  ...
  if not visible then return end -- IF ENVELOPE IS NOT VISIBLE DO NOTHING
...
function create_envelope(guid,cur_env,tab,job,duplicate)
  if tab ~= 2 then return end 
  ::JUMP::
  local env_chunk, env_name, env = get_env(guid,cur_env) -- GET POINTS AND NAME
  if not env_name then return end -- THERE IS NO ENVELOPE TRACK
  local name
  if job == "V" then 
    ...
    if time_start and time_end then reaper.DeleteEnvelopePointRange( env, time_start, time_end ) end-- REMOVE POINTS IN THAT RANGE 
  elseif job == "D" then name = naming(find_guid(guid),job,duplicate,"env",cur_env) -- CREATE DUPLICATE NAMES
  end
...
local function create_button(name,guid,chunk,ver_id)
  ...
  if not find_guid(guid) then -- IF TRACK DOES NOT EXIST CREATE FIRST VERSION
    ...
    local version = find_guid(guid)
    if not version.data then version.data = {} end
...
function on_click_function(button,name,tab,duplicate)
  ...
  for i = 1, sel_tr_count do 
    ...
    if find_guid(guid) and name == "V" then new_empty(guid) end -- FIRST TIME STORE TRACK VERSION,EVERY TIME AFTER CREATE EMPTY VERSION
    local version_name
    if name == "D" then version_name = naming(find_guid(guid),name,duplicate,"track")
    else version_name = naming(find_guid(guid),name,nil,"track") end
    ...
    if #find_guid(guid).data == 1 then -- MAKE 2 VERSION ON START (IF TRACK HAS NO VERSIONS)
      if name == "V" then goto JUMP -- IF CREATING NORMAL VERSION
      elseif name == "D" then dupli = version_name goto JUMP -- IF CREATING DUPLICATE
      end 
    end 
  end
end
...
function restore_envelope(guid,cur_env,num)
  ...
  local time_start,time_end = select_deselect_env_points(env,true) -- SELECT ALL POINTS AND RETURN START AND END OF ENVELOPE POINTS
  if VIEW == 1 then time_start,time_end = get_time_sel() end -- IF IN TIME SELECTION VIEW
  if time_start and time_end then reaper.DeleteEnvelopePointRange( env, time_start, time_end ) end-- REMOVE POINTS IN THAT RANGE     
...
function delete_items_on_track(guid)
  local track = reaper.BR_GetMediaTrackByGUID( 0,guid)
  local num_items = reaper.CountTrackMediaItems(track)  
  for i = num_items, 1, -1 do local item = reaper.GetTrackMediaItem(track,i-1) reaper.DeleteTrackMediaItem(track,item) end
end
------------------------------------------------
...
function restoreTrackItems(track, num)
  ...
  if fipm == 0 then delete_items_on_track(track_tb.guid) end
  -- VIEW TIME SELECTION VERSIONS
  if VIEW == 1 then
    if has_id(track_tb,track_tb.stored_version) then local ts_num = has_id(track_tb,track_tb.stored_version)
      for j = 1, #track_tb.data[ts_num].chunk do reaper.SetItemStateChunk(reaper.AddMediaItemToTrack(track), track_tb.data[ts_num].chunk[j], false) end
    end
  end
  -- SET ITEM CHUNK
    ...
    if reaper.BR_GetMediaTrackByGUID( 0, track_items_table[i] ) then  -- FOLDER (ITS CHUNKS ARE TRACK GUIDS (CHILDS)
      ...
      if fipm == 0 then -- IF VE ARE IN STANDARD VIEW
        ...
        if VIEW == 1 then -- TIME SELECTION VIEW 
          if track_tb.stored_version ~= track_tb.data[track_tb.data.num].ver_id then reaper.SetItemStateChunk(item, track_items_table[i], false) end -- PREVENT ADDING SAME VERSION AS SOURCE         
          if ts_item_position(item) then show_ts_version(item) else reaper.DeleteTrackMediaItem(track, item) end -- TIMESELECTION VIEW
        else
  ...
  if #track_items_table == 0 then track_tb.data.num = num end -- CHECK NUM VERSION WHEN SWITCHING EMPTY VERSIONS
end
------------------------------------------------------------
...
function validate_guid()
  for i = #TrackTB, 1, -1 do 
    if not reaper.ValidatePtr(reaper.BR_GetMediaTrackByGUID(0, TrackTB[i].guid), "MediaTrack*") then table.remove(TrackTB,i) end 
  end
  if TrackTB.groups then
    for i = #TrackTB.groups, 1, -1 do
      for j = #TrackTB.groups[i], 1, -1 do
        if not reaper.ValidatePtr(reaper.BR_GetMediaTrackByGUID(0, TrackTB.groups[i][j]), "MediaTrack*") then table.remove(TrackTB.groups[i],j) end
      end
    if #TrackTB.groups[i] == 0 then TrackTB.groups[i] = nil end 
    end
    if #TrackTB.groups == 0 then TrackTB.groups = nil end
  end
  if TrackTB == nil then TrackTB = {} end
end
------------------------------------
...
function read_from_file()
  local file=io.open(fn,"r") if not file then return end
  local content = file:read("a") if content == "" then return end
...
function comping(tbl,mousepos)
  ...
  for i = 1, #tracks do
    ...
    for i = 1 , #items do
      if get_items_in_ts(items[i]) then tsitems[#tsitems+1] = get_items_in_ts(items[i]) end
    end     
    ...
    for i = 1, #tsitems do
      ...
      if reaper.IsMediaItemSelected( tsitem ) == true then reaper.SetMediaItemSelected( tsitem, false ) end -- deselect selected item if selected (because we will select items below to trim them and to choose version on mouse click)      
      ...
      if not num then -- IF COMP VERSION DOES NOT EXISTS
        ...
        local set_item_Y = reaper.SetMediaItemInfo_Value(swipedItem, "F_FREEMODE_Y", ((ver_cnt-1) * (FIPM_item + FIPM_bar))) -- SET NEW ITEM Y
        ...
        if tr_folder ~= 1 then tr_tbl.data[num].chunk[#tr_tbl.data[num].chunk+1] = swipe_chunk[1] end -- ADD CHUNK TO CURRENT COMP
...
function get_items_for_destination(guid,mouse_pos)
  ...
  local ret  
  local cur_items = {}
  if fipm == 1 then cur_items = mute_view(tr_tbl,mouse_pos) else ret, cur_items = getTrackItems(reaper.BR_GetMediaTrackByGUID( 0, guid ))end  
  
  local items = {}
  for i = 1, #cur_items do    
    if get_time_sel() and get_items_in_ts(cur_items[i]) then items[#items+1] = get_items_in_ts(cur_items[i]) end-- TIME SELECTION AND ITEMS ARE IN IT        
  end 
 
  for i = 1, #items do
    if reaper.IsMediaItemSelected( items[i]) == true then reaper.SetMediaItemSelected( items[i], false ) end -- deselect selected item if selected (because we will select items below to trim them and to choose version on mouse click)      
...
function auto_save(last_action,current,guid,mousepos)
  ...
  if not has_undo(ignore,last_action) then 
    if last_action:find("remove tracks") then validate_guid() end -- TRACK HAS BEEN DELETED IN REAPER
    if not last_action:find("automation item") and last_action:find("item") or last_action:find("recorded media") or last_action:find("midi editor: insert notes") or last_action:find("change source media") or last_action:find("rename source media") then     
      ...
      if not find_guid(cur_track) or not find_guid(cur_track).data then return end
      ...
      if last_action:find("pencil") or last_action:find("adjust")then
        if not folder and current then tracks_tbl[#tracks_tbl+1] = reaper.GetTrackGUID(current) end  -- IF WE ARE MODIFYING CURRENT VERSION (DRAWING,FADING,CUTTING)    
      end
      ...
      if last_action:find("marquee item selection") or last_action:find("split items") or last_action:find("change media item selection")then
      ...
      for i = 1, #cur_tbl do
        ...
        if not track then return end -- IF TRACK HAS NO DATA DO NOTHING
        local _, _, _, fipm = get_track_info(track.guid)
        if last_action:find("change media item selection") then return end -- DO NOT SAVE DATA IF WE ARE ONLY CHANGING ITEM SELECTION    
      ...
      if last_action:find("delete items") then test = nil end
    end
    ...
    if last_action:find("envelope") then
      ...
      if not tbl or not tbl[cur_env] then return end -- IF ENVELOPE VERSION DOES NOT EXIST DO NOT STORE ANYTHING 
